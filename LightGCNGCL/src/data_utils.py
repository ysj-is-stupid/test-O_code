...
def create_dataset(args):
    ...
    elif args.dataset in ['mashup', 'api', 'gowalla', 'amazon-book', 'yelp2018']:
            ...
            with open(self.test_file) as f:
                for l in f.readlines():
                    if len(l) > 1:
                        print(l[0])
                        l = l.strip('\n').split(' ')
                        test_users.append(int(l[0]))
                        # if l[1:] == ['']:
                        #     items = []
                        #     ground_true_items.append(items)
                        # else:
                        #     items = [int(i) for i in l[1:]]
                        ground_true_items.append(int(l[1]))
            test_data = torch.tensor([test_users, ground_true_items])
            return test_users, ground_true_items,test_data
