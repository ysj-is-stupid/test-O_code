...
class Model(nn.Module):
	def __init__(self):
		...
		self.uEmbeds = nn.Parameter(init(torch.empty(6298, 128)))
		self.iEmbeds = nn.Parameter(init(torch.empty(1609, 128)))
	...
	def loss_graphcl(self, x1, x2, users, items):
		T = args.temp
		user_embeddings1, item_embeddings1 = torch.split(x1, [6298, 1609], dim=0)
		user_embeddings2, item_embeddings2 = torch.split(x2, [6098, 1609], dim=0)
...
class vgae(nn.Module):
	...
	def generate(self, data, edge_index):
